...
from espn_scraper.items import NFL_QB_Game_2015
...
class EspnSpider(Spider):
    ...
    start_urls = ['http://espn.go.com/nfl/player/gamelog/_/id/14874/andrew-luck',
                  'http://espn.go.com/nfl/player/gamelog/_/id/2330/tom-brady',
                  'http://espn.go.com/nfl/player/gamelog/_/id/14877/nick-foles',
                  'http://espn.go.com/nfl/player/gamelog/_/id/2576980/marcus-mariota'
                  ]
    ...
    def parse(self, response):
        ...
        for game_row in games_table:
            ...
            if game_row[0] == 'REGULAR SEASON STATS':
                ...
                loader.add_value('pass_completions', int(game_row[1]))
                loader.add_value('pass_attempts', int(game_row[2]))
                loader.add_value('pass_yards', int(game_row[3]))
                loader.add_value('comp_percentage', float(game_row[4]))
                loader.add_value('avg_yards_per_pass', float(game_row[5]))
                loader.add_value('longest_pass', int(game_row[6]))
                loader.add_value('pass_tds', int(game_row[7]))
                loader.add_value('interceptions', int(game_row[8]))
                loader.add_value('qb_rating', float(game_row[9]))
                loader.add_value('passer_rating', float(game_row[10]))
                loader.add_value('rush_attempts', int(game_row[11]))
                loader.add_value('rush_yards', int(game_row[12]))
                loader.add_value('avg_yards_per_rush', float(game_row[13]))
                loader.add_value('longest_rush', int(game_row[14]))
                loader.add_value('rush_tds', int(game_row[15]))
            ...
            # Handle regular season individual game row (ignoes two header rows)
                ...
                loader.add_value('pass_completions', int(game_row[3]))
                loader.add_value('pass_attempts', int(game_row[4]))
                loader.add_value('pass_yards', int(game_row[5]))
                loader.add_value('comp_percentage', float(game_row[6]))
                loader.add_value('avg_yards_per_pass', float(game_row[7]))
                loader.add_value('longest_pass', int(game_row[8]))
                loader.add_value('pass_tds', int(game_row[9]))
                loader.add_value('interceptions', int(game_row[10]))
                loader.add_value('qb_rating', float(game_row[11]))
                loader.add_value('passer_rating', float(game_row[12]))
                loader.add_value('rush_attempts', int(game_row[13]))
                loader.add_value('rush_yards', int(game_row[14]))
                loader.add_value('avg_yards_per_rush', float(game_row[15]))
                loader.add_value('longest_rush', int(game_row[16]))
                loader.add_value('rush_tds', int(game_row[17]))
    ...
    def parse_rb(self, response):
        ...
        # //*[@id="content"]/div[6]/div[1]/div/div[4]/div/table
        

        # rows = response.xpath('//*[@id="content"]/div[6]/div[1]/div/div[3]/div/table/tbody/tr[3]')
        # test = response.xpath('//*[@id="content"]/div[6]/div[1]/div/div[3]/div/table/tbody')
        # print(test)
